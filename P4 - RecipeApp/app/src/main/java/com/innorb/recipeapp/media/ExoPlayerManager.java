...
public class ExoPlayerManager implements Player.EventListener {
    ...
    private void progressBar(final ExoPlayer exoPlayer) {
        ...
        new Thread(() -> {
            while (mProgressStatus < mProgressBar.getMax()) {
                ...
                try {
                    Thread.sleep(Constants.EXO_PROGRESSBAR_DELAY);
    ...
    public void initializeMediaSession() {
        StepActivity.mMediaSession = new MediaSessionCompat(mContext, Constants.EXO_PLAYER_MANAGER_TAG);
        StepActivity.mMediaSession.setFlags(
                MediaSessionCompat.FLAG_HANDLES_MEDIA_BUTTONS |
                        MediaSessionCompat.FLAG_HANDLES_TRANSPORT_CONTROLS |
                        Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);
        StepActivity.mMediaSession.setMediaButtonReceiver(null);
        ...
        StepActivity.mMediaSession.setPlaybackState(mStateBuilder.build());
        StepActivity.mMediaSession.setCallback(new MySessionCallback());
        StepActivity.mMediaSession.setActive(true);
    }
    ...
    private void showNotification(PlaybackStateCompat state) {
        ...
        NotificationManager notificationManager = (NotificationManager) mContext.getSystemService(NOTIFICATION_SERVICE);
        if(notificationManager!=null){
            notificationManager.notify(Constants.NOTIFICATION_CHANNEL_ID, Constants.NOTIFICATION_ID, builder.build());
        }
    ...
    public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
        ...
        if ((StepActivity.mMediaSession != null) && (mStateBuilder != null)) {
            StepActivity.mMediaSession.setPlaybackState(mStateBuilder.build());
