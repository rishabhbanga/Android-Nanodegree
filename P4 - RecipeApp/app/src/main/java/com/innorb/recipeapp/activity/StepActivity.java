...
import com.innorb.recipeapp.fragment.DetailStepFragment;
...
import com.innorb.recipeapp.media.ExoPlayerManager;
...
public class StepActivity extends BaseActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            setRecipeId(savedInstanceState.getInt(Constants.BUNDLE_RECIPE_ID));
            setNavigationId(savedInstanceState.getInt(Constants.BUNDLE_DETAIL_STEP_NAVIGATION_ID));
            setVideoUri(savedInstanceState.getString(Constants.BUNDLE_DETAIL_STEP_VIDEOURI));
            setThumbnailURL(savedInstanceState.getString(Constants.BUNDLE_DETAIL_STEP_THUMBNAILURL));
            setShortDescription(savedInstanceState.getString(Constants.BUNDLE_DETAIL_STEP_SHORT_DESCRIPTION));
            setDescription(savedInstanceState.getString(Constants.BUNDLE_DETAIL_STEP_DESCRIPTION));
            sIdData = savedInstanceState.getInt(Constants.BUNDLE_DETAIL_STEP_ID);
        } else if (intent != null) {
            sIdData = intent.getIntExtra(Constants.EXTRA_DETAIL_STEP_ID, -1);
        ...
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            startFragmentTablet(getRecipeId(), Constants.TAB_ORDER_STEP);
        ...
        if (Timber.treeCount() < Constants.DEFAULT_TIMBER_COUNT) {
            if (!Utility.isTablet(getApplicationContext())) {
        ...
        if (savedInstanceState != null) {
            if (mExoPlayerManager != null) {
                mExoPlayerManager.setResume(
                        savedInstanceState.getInt(Constants.BUNDLE_EXOPLAYER_WINDOW, C.INDEX_UNSET),
                        savedInstanceState.getLong(Constants.BUNDLE_EXOPLAYER_POSITION, C.TIME_UNSET));
                mExoPlayerManager.setAutoPlay(
                        savedInstanceState.getBoolean(Constants.BUNDLE_EXOPLAYER_AUTOPLAY, false));
            }
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        if (savedInstanceState.getString(Constants.EXTRA_RECIPE_NAME) != null) {
            setRecipeName(savedInstanceState.getString(Constants.EXTRA_RECIPE_NAME));
        }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        if (mExoPlayerManager != null) {
            outState.putInt(Constants.BUNDLE_EXOPLAYER_WINDOW, mExoPlayerManager.getResumeWindow());
            outState.putLong(Constants.BUNDLE_EXOPLAYER_POSITION, mExoPlayerManager.getResumePosition());
            outState.putBoolean(Constants.BUNDLE_EXOPLAYER_AUTOPLAY, mExoPlayerManager.isAutoPlay());
        }

        outState.putInt(Constants.BUNDLE_RECIPE_ID, getRecipeId());
        outState.putString(Constants.BUNDLE_RECIPE_NAME, getRecipeName());
        outState.putInt(Constants.BUNDLE_DETAIL_STEP_ID, sIdData);
        outState.putInt(Constants.BUNDLE_DETAIL_STEP_NAVIGATION_ID, mNavigationId);
        outState.putString(Constants.BUNDLE_DETAIL_STEP_VIDEOURI, getVideoUri());
        outState.putString(Constants.BUNDLE_DETAIL_STEP_THUMBNAILURL, getThumbnailURL());
        outState.putString(Constants.BUNDLE_DETAIL_STEP_SHORT_DESCRIPTION, getShortDescription());
        outState.putString(Constants.BUNDLE_DETAIL_STEP_DESCRIPTION, getDescription());
    ...
    public void onFragmentInteraction(int id, int position) {
        ...
        intent.putExtra(Constants.EXTRA_NAVIGATION_TYPE, R.string.device_type_tablet);
    ...
    private void destroyVideoBackground() {
        if (mMediaSession != null) {
            ...
            if (notificationManager != null) {
                notificationManager.cancel(Constants.NOTIFICATION_CHANNEL_ID, Constants.NOTIFICATION_ID);
            }
        }
    }
    ...
    private void navigationIntent(int navigationType) {
        ...
        intent.putExtra(Constants.EXTRA_RECIPE_ID, getRecipeId());
        intent.putExtra(Constants.EXTRA_NAVIGATION_TYPE, navigationType);
        intent.putExtra(Constants.EXTRA_RECIPE_NAME, getRecipeName());
        intent.putExtra(Constants.EXTRA_DETAIL_STEP_ID, sIdData);
        startActivity(intent);
    }
    ...
    private void startFragmentTablet(int index, int orderTab) {
        if (Utility.isTablet(mContext) && (index >= 0)) {
            ...
            if (PrefManager.isGeneralSettings(this, getString(R.string.pref_tab_layout))) {
                ...
                bundle.putString(Constants.EXTRA_RECIPE_NAME, getRecipeName());
